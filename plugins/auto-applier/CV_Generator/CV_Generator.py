...
SYSTEM_MSG = r"""You are a CV generator that takes four inputs—(1) my extended CV (source facts), (2) my current LaTeX CV template to edit, (3) fixed prompt rules/style constraints, and (4) a full job description—and returns only a complete, compile‑ready LaTeX document that starts with \documentclass and ends with \end{document}, with no explanations, code fences, or extra text before/after."""
...
def create_latex_CV( job_description: str) -> None:
    load_dotenv()
    # Load API key from environment (dotenv already called above)
...
if __name__ == "__main__":
    job = read(JOB_FILE)
    create_latex_CV("Airbus_Stage", job)