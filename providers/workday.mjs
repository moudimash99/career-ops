 ...
 * are optional; omitting them reproduces the unfiltered behaviour exactly.
 * Values are normalized to arrays of strings, the only shape the API accepts.
 *
 * This is the FIRST line of defence against the offset ceiling and the facet
 * split below is the second: narrowing the board server-side is cheaper and
 * more precise than recovering a clamped one after the fact, but a tenant that
 * is still over the ceiling *after* narrowing gets split as well. The two
 * compose — see the `excluded` seed in fetch() for the one way they must not.
 */
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    // The entry's own narrowing underlies EVERY request — the unfaceted root
    // crawl and each split slice alike. Slice facets layer on top, so a split
    // narrows the scoped board further and can never widen it back out.
    const entryFacets = resolveAppliedFacets(entry);
    const searchText = typeof entry.search_text === 'string' ? entry.search_text : '';
    const makeBody = (offset, appliedFacets) => JSON.stringify({
      limit: PAGE_SIZE,
      offset,
      searchText,
      appliedFacets: { ...entryFacets, ...appliedFacets },
    });
    ...
    if (root.clamped) {
      ...
      // Seeded with the entry's own facet parameters, never empty: if the
      // split chose a facet the entry already pinned, the slice's value would
      // REPLACE that pin (same key in the makeBody spread) and silently widen
      // the scan back past the narrowing — e.g. a locationCountry: [France]
      // entry re-sliced into locationCountry: [Brazil]. Excluding them keeps
      // the pin invariant across every level of the split.
      await split(root, {}, 0, Object.keys(entryFacets));
      jobs = out;
