...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    // Fork: an entry may pin its own facets / search text (see resolveAppliedFacets).
    // They scope every request, including each facet-split slice below.
    const baseFacets = resolveAppliedFacets(entry);
    const searchText = typeof entry.search_text === 'string' ? entry.search_text : '';
    const makeBody = (offset, appliedFacets) => JSON.stringify({ limit: PAGE_SIZE, offset, searchText, appliedFacets: { ...baseFacets, ...appliedFacets } });
    ...
    if (root.clamped) {
      ...
      // Facets the entry pinned are already applied to every slice, so they are never split on.
      await split(root, {}, 0, Object.keys(baseFacets));
      jobs = out;
