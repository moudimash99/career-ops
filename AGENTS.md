...
- **User Layer (NEVER auto-updated; personalization goes HERE):** `cv.md`, `config/profile.yml`, `modes/_profile.md`, `modes/_custom.md`, `article-digest.md`, `config/apply-answers.yml`, `config/apply-essays.yml`, `portals.yml`, `data/*`, `documents/*`, `reports/*`, `output/*`, `interview-prep/*`
...
| `letter-write.mjs` | agy writes a cover letter from ONE pasted context (the "Letter writing" rules in `modes/_custom.md` + `voice-dna.md` + approved tone examples from `config/apply-essays.yml` + `cv.md` + posting); `lib/letter-check.mjs` checks it (language, length, AI tells, claims vs cv.md, copies of recent letters, layout leftovers) with one retry, else refuses (exit 2). `--format form|pdf`, `--pdf` renders a plain letter, `--context-only` + `--check` for Free Motion, `--sample 3 --since <t>` prints letters to skim. A changed prompt ramps up 20% → 50% → 100% (`lib/letter-rollout.mjs`) |
| `lib/letter-experiment.mjs` | Letter arm per posting (none 15 / short 35 / full 50, `letter_experiment.weights`), same ledger logic as the CV experiment, `data/letter-experiment.tsv` |
