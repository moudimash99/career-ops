...
<p align="center" dir="rtl">
  تستخدم الشركات الذكاء الاصطناعي لتصفية المرشحين. <strong>أنا ببساطة أعطيت المرشحين الذكاء الاصطناعي ليختاروا هم الشركات المناسبة.</strong>
</p>
...
| **مسودات رسائل التقديم** | رسائل رسمية لمسؤول التوظيف أو للإحالة أو للتقديم البارد من تقرير أو وصف ملصق، مع سطر الموضوع، وقائمة المرفقات، ونقاط ملاءمة موثقة بمصادرها، وكتلة تواصل مستمدة من ملفك. مسودة فقط. |
...
> 💡 `npx` يأتي مع [Node.js](https://nodejs.org): يشغّل المثبّت مرة واحدة
> دون تثبيت أي شيء بشكل عام. لا يوجد Node بعد؟ ثبّته أولًا.
> (تستخدم بالفعل واجهة Claude Code / Gemini / Codex؟ إذن لديك بالفعل.)

يستنسخ هذا أحدث إصدار في `./career-ops` ويثبّت الاعتماديات. ثم:
...
**عند التشغيل الأول، يرشدك career-ops خلال الإعداد (سيرتك الذاتية وملفك والأدوار المستهدفة) بمجرد الدردشة. لا شيء يُحرَّر يدويًا.**

<details>
<summary><b>تفضّل الإعداد اليدوي؟ (git clone)</b></summary>
...
يثبّت هذا الملف التنفيذي `career-ops` بشكل عام لتتمكن من تشغيله مباشرة بدلًا من `npx`. على عكس `npx @santifer/career-ops init` (الذي يهيئ مجلد مشروع)، يمنحك التثبيت العام أمر `career-ops` دائمًا متاحًا في أي مكان في الطرفية.

**أيهما تستخدم؟**
- `npx @santifer/career-ops init`: الأفضل للاستخدام الأول؛ ينشئ مجلد مشروع مخصصًا.
- `npm i -g @santifer/career-ops`: الأفضل عندما يكون لديك مجلد مشروع وتريد تشغيل أوامر career-ops مباشرة.
...
/career-ops email     → مسودة رسالة تقديم رسمية (مسودة فقط)
...
يدعم career-ops واجهة Antigravity CLI أصلًا، بالطريقة نفسها التي يدعم بها Claude Code و OpenCode. جميع أوامر الشرطة المائلة متاحة عبر نقطة دخول المهارة المشتركة، باستخدام منطق التقييم نفسه في `modes/*.md`.

نقلت Google وصول المستهلكين إلى Gemini CLI نحو Antigravity CLI. أصبح `GEMINI.md` الآن حارس توافق بلا تأثير، حتى لا يكرر Antigravity تعليمات المشروع الكاملة عندما يقرأ `AGENTS.md` و`GEMINI.md` معًا.

### Antigravity CLI الأصلي
...
يدعم career-ops واجهة Codex عبر الموجّه المشترك نفسه، لكن نموذج الاستدعاء يختلف عن الواجهات التي تسجّل أوامر الشرطة المائلة تلقائيًا. للدليل الكامل انظر [docs/CODEX.md](docs/CODEX.md).
...
أوامر الشرطة المائلة غير مضمونة في Codex. إذا لم يكن `/career-ops` متاحًا، اطلب من Codex تشغيل الوضع مباشرة بلغة عادية:
...
## تكامل Grok Build CLI

يدعم career-ops واجهة Grok Build CLI أصلًا، بالطريقة نفسها التي يدعم بها Claude Code و OpenCode. يُحمَّل `AGENTS.md` تلقائيًا كقواعد للمشروع، وجميع أوامر الشرطة المائلة متاحة عبر نقطة دخول المهارة المشتركة.

### Grok Build CLI الأصلي
...
**مراكز الاتصال:** Ada, LivePerson, Sierra, Decagon, Talkdesk, Genesys
**الشركات الكبرى:** Salesforce, Twilio, Gong, Dialpad
**LLMOps:** Langfuse, Weights & Biases, Lindy, Cognigy, Speechmatics
**الأتمتة:** n8n, Zapier, Make.com
**أوروبا:** Factorial, Attio, Tinybird, Clarity AI, Travelperk

**لوحات الوظائف التي يتم البحث فيها:** أكثر من 55 وحدة مزوّد تغطي واجهات ATS البرمجية، وتغذيات اللوحات الكاملة، وتغذيات XML/RSS، وتغذيات markdown، ومحللات محلية. انظر [لوحات الوظائف المدعومة](docs/SUPPORTED_JOB_BOARDS.md) للجدول الكامل.

افتراضيًا، يثق `node scan.mjs` (أي `npm run scan`) بما تعيده كل تغذية ATS. بعض الشركات تترك إعلانات قديمة في واجهتها البرمجية العامة حتى بعد إغلاق الوظيفة، فقد تتسرب تلك الإدخالات المنتهية إلى `pipeline.md`. مرّر `--verify` لتشغيل Playwright بعد المرور بالواجهة البرمجية وإسقاط الإعلانات المنتهية قبل وصولها إلى خط العمل:

```bash
node scan.mjs --verify          # zero-token discovery + Playwright liveness check
```

التحقق متسلسل ويعمل فقط على العروض الجديدة (بعد إزالة التكرار)، لذا تبقى التكلفة محدودة.

## لوحة تحكم TUI

تتيح لك لوحة التحكم الطرفية المدمجة تصفح خط عملك بصريًا:
...
## دليل البيانات الخارجي (اختياري)

افتراضيًا، تعيش بيانات طبقة المستخدم (مثل `cv.md` و`portals.yml` ومجلدات `data/` و`reports/` و`output/`) داخل مجلد جذر المشروع.

لفصل بياناتك الشخصية عن الكود (مما يسهّل تبديل الفروع أو سحب التحديثات أو اختبار عدة ملفات)، يمكنك تهيئة دليل بيانات خارجي وفق الأولوية التالية:

1. **متغيرات البيئة:** اضبط متغير البيئة `CAREER_OPS_ROOT` أو `CAREER_OPS_DATA_DIR`:
   ```bash
   export CAREER_OPS_ROOT=~/my-career-data
   ```
2. **ملف العلامة:** أنشئ ملف `.career-ops-data` في جذر المستودع يحتوي على مسار دليل بياناتك.
3. **الافتراضي:** جذر المستودع.

بعد تحديد المسار، تُحلّ جميع ملفات المستخدم وتُكتب نسبةً إلى ذلك المجلد، بينما تستمر ملفات الأوامر النصية والسكربتات في الحلّ نسبةً إلى المستودع.
...
- **الوكيل**: واجهة سطر أوامر للبرمجة بالذكاء الاصطناعي مع مهارات وأوضاع مشتركة (`AGENTS.md` + غلاف لكل واجهة)
- **PDF**: Playwright + قالب HTML
- **خطابات التقديم**: قالب HTML + Playwright (PDF بحجم A4، خط العمل نفسه المستخدم للسير الذاتية)
- **الماسح**: Playwright + Greenhouse API + WebSearch
...
**أدفع اشتراك Claude Pro/Max لكن career-ops يستهلك رصيد API. لماذا؟**
لأن وجود `ANTHROPIC_API_KEY` في بيئتك يتقدم على اشتراكك المسجّل دخوله: تستخدم الواجهة المفتاح وتُحاسِب لكل رمز. شغّل `echo $ANTHROPIC_API_KEY`، وإن طبع شيئًا فأزله من ملف تعريف الصدفة، ثم أعد تشغيل الطرفية وشغّل `/login`. وضع المعالجة الجماعية هو الاستثناء، لأن عمّال `claude -p` لا يستخدمون تسجيل الدخول التفاعلي: شغّل `claude setup-token` مرة واحدة وصدّر الناتج كـ `CLAUDE_CODE_OAUTH_TOKEN`. الشرح الكامل في [docs/RUNNING_ON_A_BUDGET.md](docs/RUNNING_ON_A_BUDGET.md#2b-already-paying-for-a-subscription-make-sure-you-are-using-it).

**مع أي واجهات ذكاء اصطناعي يعمل career-ops؟**
يعمل career-ops على أي واجهة رئيسية للبرمجة بالذكاء الاصطناعي (Claude Code و Codex و Gemini / Antigravity و OpenCode و Grok و Qwen وغيرها) عبر معيار Agent Skill Standard المفتوح، فلا يُقيَّد أبدًا بمزوّد واحد. استخدم الواجهة التي لديك بالفعل.

**كيف أثبّت career-ops على Windows؟**
...
**career-ops أداة محلية مفتوحة المصدر، وليست خدمة مستضافة.** باستخدامك لهذا البرنامج، فإنك تقر بما يلي:

1. **أنت تتحكم ببياناتك.** تبقى سيرتك الذاتية وبيانات الاتصال وبياناتك الشخصية على جهازك وتُرسل مباشرة إلى مزوّد الذكاء الاصطناعي الذي تختاره (Anthropic و OpenAI وغيرهما). نحن لا نجمع أيًا من بياناتك ولا نخزّنها ولا نطّلع عليها.
2. **أنت تلتزم بشروط خدمة الأطراف الخارجية.** يجب أن تستخدم هذه الأداة وفق شروط خدمة بوابات التوظيف التي تتعامل معها (Greenhouse و Lever و Workday و LinkedIn وغيرها). لا تستخدم هذه الأداة لإرسال رسائل مزعجة إلى جهات التوظيف أو لإغراق أنظمة ATS.
3. **لا ضمانات.** التقييمات توصيات، لا حقائق. قد تختلق نماذج الذكاء الاصطناعي مهارات أو خبرات. لا يتحمل المؤلفون أي مسؤولية عن نتائج التوظيف أو الطلبات المرفوضة أو تقييد الحسابات أو أي تبعات أخرى.

انظر [LEGAL_DISCLAIMER.md](LEGAL_DISCLAIMER.md) للتفاصيل الكاملة. يُقدَّم هذا البرنامج بموجب [رخصة MIT](LICENSE) "كما هو"، دون أي ضمان من أي نوع.
...
كل من ساهم بكود أو وثائق أو ترجمات أو اختبارات مدرج في
[CONTRIBUTORS.md](CONTRIBUTORS.md)، بما في ذلك المساهمات غير البرمجية التي
لا يستطيع الرسم البياني أعلاه إظهارها.

هل حصلت على وظيفة باستخدام career-ops؟ [شارك قصتك!](https://github.com/career-ops-hq/career-ops/issues/new?template=i-got-hired.yml)

## الرخصة والعلامة التجارية

الكود مرخّص بموجب [MIT](LICENSE). أما اسم "career-ops" وعلامته
التجارية فتحكمهما [سياسة العلامة التجارية](TRADEMARK.md): متساهلة
