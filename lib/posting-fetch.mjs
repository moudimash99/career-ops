// @ts-check
/**
 * lib/posting-fetch.mjs — the text of one HelloWork or LinkedIn posting,
 * without a browser. Those two boards send titles only in their search
 ...
 *
 *   HelloWork  the posting page's schema.org JobPosting block (JSON-LD),
 *              which job sites publish for search engines
 *   LinkedIn   the guest endpoint liveness-api.mjs already uses
 *              (jobs-guest/jobs/api/jobPosting/{id}), paced 3.5 s apart
 *
 * Only these two hosts, over https: the URLs come from scraped data. Returns
 * null when the page has no readable text; the caller then scores the title.
 */

import { htmlToText } from '../providers/_html-to-text.mjs';
import { resolveAtsApi, throttleProviderRequest } from '../liveness-api.mjs';

const HELLOWORK_THROTTLE_MS = 1_500;
...
/**
 * @param {string} url - a HelloWork or LinkedIn posting URL
...
export async function fetchPostingText(url, { fetchImpl = fetch, throttle = throttleProviderRequest } = {}) {
  ...
  } else if (isHellowork(url)) {
    await throttle('hellowork', HELLOWORK_THROTTLE_MS);
    target = url;
    read = jobPostingText;
  } else {
    return null;
