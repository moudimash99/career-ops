...
export const FORM_INVENTORY_SCRIPT = `() => {
  ...
  // Names that mean "this is a consent question", used only to exonerate a
  // field from the honeypot heuristics. Kept separate from the answer-side
  // consent matching: this list only has to be right about what is NOT a trap.
  const CONSENT_NAME = /agree|consent|accept|acknowledg|privacy|policy|gdpr|terms|opt.in|j.accepte|je reconnais|je d.clare|conditions|politique|confidentialit|rgpd|donn.es/i;

  const isHoneypot = (el, label) => {
    const name = String(label || '').trim();

    // 1. The label ASKS to be left alone, in whatever language it asks. This
    //    one convicts on its own and nothing below can overturn it.
    if (LEAVE_BLANK.test(name)) return true;
    ...
    // 2. A field with a real accessible name is a real question, however it is
    //    rendered. This is the discriminator, and it took two false positives
    //    on live applications to find it: a consent checkbox is routinely
    //    invisible (opacity 0, or parked off-screen by the sr-only pattern)
    //    with its text painted next to it, and both times the field was
    //    dropped as a honeypot — silently, so a REQUIRED consent went
    //    unanswered and the submit would have failed naming nothing.
    //
    //    An earlier attempt keyed on "does it have a VISIBLE bound label",
    //    which is weaker: it depends on the label being reachable through
    //    labels[0] / label[for] / an ancestor, and a component library that
    //    renders the text in a sibling node defeats it while still showing a
    //    perfectly readable sentence to the user. What a honeypot never has is
    //    something meaningful to say: its accessible name is empty, or it is
    //    the instruction not to fill it in. A sentence about a privacy policy
    //    is not a trap.
    if (name.length >= 12) return false;

    //    Length alone is not quite enough: "J.accepte" is nine characters and
    //    is one of the commonest consent labels there is. Consent vocabulary
    //    exonerates too, in both languages these forms are written in. A trap
    //    never asks you to agree to something.
    if (CONSENT_NAME.test(name)) return false;

    // 3. No meaningful name. Now the hiding signals mean what they look like.
    const rect = el.getBoundingClientRect();
    ...
    // 4. Out of the tab order with nothing to say. A real question is always
    //    reachable by keyboard.
    const unnamed = !name
      && !el.getAttribute('aria-label')
      && !el.getAttribute('aria-labelledby');
    if (unnamed && el.getAttribute('tabindex') === '-1') return true;
  ...
  // Consent banners do not all speak in "accept" and "reject". A live one
  // offered "No, thanks" / "I choose" / "OK for me" and was not recognised as
  // consent at all, so the page reported no wall and the caller was handed the
  // widget's container as if it were a form field. These are phrasings, not
  // vendors: any banner may use them.
  const CONSENT_RE = /cookie|consent|consentement|préférences|preferences|accept all|reject all|zustimmen|akzeptieren|aceptar|tout accepter|tout refuser|continuer sans|refuser|decline|ablehnen|rechazar|ok for me|no,? ?thanks|i choose|only essential|essential only|n[ée]cessaires uniquement|je choisis/i;
  // Declining clears the wall without agreeing to tracking, so the wording that
  // means "no" is marked and the caller can prefer it. Recognising it matters
  // twice over: one live wall's decline button read "Continuer sans accepter",
  // which the accept-only vocabulary above did not match at all.
  const CONSENT_DECLINE_RE = /continuer sans|reject all|tout refuser|refuser|decline|ablehnen|rechazar|no,? ?thanks|only essential|essential only|n[ée]cessaires uniquement/i;
  const consentNodes = deepQueryAll('button, [role="button"], a')
    .filter(visible)
    .filter((b) => { const t = readableText(b); return t && CONSENT_RE.test(t); })
    .slice(0, 8);
  const consentButtons = consentNodes.map((b) => ({
    selector: selectorFor(b),
    text: readableText(b),
    declines: CONSENT_DECLINE_RE.test(readableText(b)),
  }));

  // Is one of those buttons sitting in something that COVERS the page? This is
  // the difference between a wall and a "Cookie settings" link in the footer,
  // and it has to be measured, not inferred from "the page has no controls".
  // A posting keeps a visible widget or two of its own underneath the overlay
  // — one live advert kept a skills picker — so counting controls declared a
  // full-screen modal to be no wall at all.
  //
  // Vendor-neutral test: an ancestor that is fixed-position or a modal dialog,
  // covering a quarter of the viewport or more.
  const viewportArea = Math.max(1, window.innerWidth * window.innerHeight);
  const coversPage = (el) => {
    for (let n = el; n && n !== document.documentElement; n = n.parentElement || (n.getRootNode() || {}).host) {
      let style;
      try { style = getComputedStyle(n); } catch { break; }
      const modal = (n.tagName === 'DIALOG' && n.hasAttribute('open'))
        || n.getAttribute?.('aria-modal') === 'true'
        || n.getAttribute?.('role') === 'dialog';
      if (style.position === 'fixed' || modal) {
        const r = n.getBoundingClientRect();
        if ((r.width * r.height) / viewportArea >= 0.25) return true;
      }
    }
    return false;
  };
  ...
  const counts = {
    ...
    // The same counts restricted to what a person can actually see and use.
    // A job POSTING carries a surprising number of hidden controls: job-alert
    // widgets, a dormant account-creation form, the cookie banner's own
    // checkboxes. One live advert reported 17 fields and 3 groups with every
    // single field \`visible: false\`. Any decision made from the raw counts
    // reads that page as an application form and fills a mailing list, so the
    // raw counts stay (they describe the DOM) and every JUDGEMENT below is
    // made from these. A honeypot is excluded for the same reason
    // \`pendingWork\` excludes it: it is bait, deliberately unseeable, and
    // counting it is how you get talked into filling it.
    visibleFields: fields.filter((f) => f.visible && !f.honeypot).length,
    visibleGroups: groups.filter((g) => g.visible).length,
  ...
  // Either an overlay is measurably covering the page, or there is nothing
  // else on it at all and only cookie buttons to press.
  const consentWall = consentButtons.length > 0
    && (consentNodes.some(coversPage)
      || (counts.visibleFields === 0 && counts.visibleGroups === 0 && counts.uploads === 0));
  ...
  return {
    ...
    frames,
    consentWall,
    // Always handed back, not only when the banner is in the way.
    //
    // The wall flag answers "is this blocking the form"; the BUTTONS are
    // useful regardless. A corner banner covers nothing, so the wall flag is
    // correctly false — but suppressing its buttons left the caller unable to
    // dismiss it at all, and one live run spent every turn trying to interact
    // with the widget's container as though it were a form field, because that
    // was the only thing it had been offered.
    consentButtons,
    counts,
  };
}`;
...
export function pendingWork(inventory) {
  ...
  for (const f of inventory?.fields ?? []) {
    ...
    const entry = {
      ...
      // Carried on the entry as well as implied by which list it lands in.
      // A caller filtering the combined list — form scoping, say — otherwise
      // has no way to tell a required control from an optional one, and
      // silently drops the required ones it meant to protect.
...
export function looksLikeApplicationForm(inventory) {
  ...
  // Counted on what is on SCREEN. A posting's hidden furniture — job-alert
  // widgets, a dormant account form, the cookie banner's checkboxes — cleared
  // this threshold on its own: one advert reported 17 fields and 3 groups, all
  // of them invisible, and was read as the application form. The form was
  // behind the Apply link the whole time.
  //
  // `visibleFields` is absent from an inventory captured before it existed, so
  // fall back to the raw counts rather than calling every old capture "not a
  // form" — the old reading, not a new wrong one.
  const fields = c.visibleFields ?? c.fields;
  const groups = c.visibleGroups ?? c.groups;
  return (fields + groups) >= 4;
}
/**
...
export function readiness(inventory) {
  ...
  // Nothing left to fill is the same reading as nothing to fill in the first
  // place, and on a posting the second is what is true. Left alone, this
  // returned `ready: true` on a job advert — an invitation to press Submit on
  // a page with no application on it.
  //
  // Deliberately narrow. Too little on screen to be a form is ALONE a
  // legitimate state (a wizard's review step has no fields and a real Submit),
  // so it only blocks when the page also offers a way IN — an Apply link —
  // which is the posting's signature and never a review step's.
  if (!looksLikeApplicationForm(inventory) && (inventory?.entryPoints ?? []).length > 0) {
    blockers.push(`form not open yet: entry point "${inventory.entryPoints[0].text}" not followed`);
  }
  if (inventory?.consentWall) blockers.push('consent wall covering the page');
