...
export const FORM_INVENTORY_SCRIPT = `() => {
  ...
  const controls = deepQueryAll('input:not([type=hidden]), select, textarea, [contenteditable="true"]');
  ...
  const groups = Object.entries(groupMembers).map(([key, members]) => ({
    ...
    required: members.some((m) => m.hasAttribute('required') || m.getAttribute('aria-required') === 'true'),
...
export function pendingWork(inventory) {
  ...
  for (const f of inventory?.fields ?? []) {
    ...
    const answered = f.role === 'checkbox' ? f.checked === true : String(f.value ?? '') !== '';
