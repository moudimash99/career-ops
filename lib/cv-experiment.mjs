...
export const CALLBACK_STATES = new Set(['Responded', 'Interview', 'Offer', 'Hired']);
// Below this many sends an arm's rate is noise; the report says so.
export const MIN_SAMPLE = 30;

// The experiment's shape. The CV experiment is the default; other factors
// (the letter experiment in lib/letter-experiment.mjs) pass their own spec
// and reuse the same ledger logic with their own arms, weights and file.
export const CV_SPEC = { arms: ARMS, defaultWeights: DEFAULT_WEIGHTS, profileKey: 'cv_experiment', ledger: LEDGER_RELATIVE_PATH };

export function ledgerPath({ logPath, root, spec = CV_SPEC } = {}) {
  const base = root ?? getCareerOpsRoot();
  if (logPath) return isAbsolute(logPath) ? logPath : join(base, logPath);
  return join(base, spec.ledger);
}

const cell = (v) => (v === null || v === undefined ? '' : String(v).replace(/[\t\r\n]+/g, ' ').trim());

/** Weights from profile.yml → <spec.profileKey>.weights, falling back per arm. */
export function readWeights(root, spec = CV_SPEC) {
  const path = join(root ?? getCareerOpsRoot(), 'config', 'profile.yml');
  let custom = {};
  try {
    if (existsSync(path)) custom = yaml.load(readFileSync(path, 'utf-8'))?.[spec.profileKey]?.weights || {};
  } catch { /* a broken profile must not stop an application; use defaults */ }
  const out = {};
  for (const arm of spec.arms) {
    const w = Number(custom[arm]);
    out[arm] = Number.isFinite(w) && w >= 0 ? w : spec.defaultWeights[arm];
  }
  return out;
}

/** Weighted draw. `rng` returns [0, 1). */
export function drawArm(weights = DEFAULT_WEIGHTS, rng = Math.random, arms = ARMS) {
  const total = arms.reduce((s, a) => s + (weights[a] || 0), 0);
  if (total <= 0) throw new Error('experiment weights sum to zero');
  let x = rng() * total;
 ...
 */
export async function assignArm(url, { report = null, company = '', role = '', forceArm = null, rng, root, logPath, spec = CV_SPEC } = {}) {
  const urlKey = normalizeUrl(url);
  if (!urlKey) throw new Error(`unkeyable URL: ${url}`);
  if (forceArm && !spec.arms.includes(forceArm)) throw new Error(`unknown arm "${forceArm}" (${spec.arms.join(', ')})`);
  const path = ledgerPath({ root, logPath, spec });
  return withPipelineLock(path, async () => {
    ...
    const arm = forceArm || drawArm(readWeights(root, spec), rng, spec.arms);
...
async function appendEvent(url, event, detail, { root, logPath, spec = CV_SPEC } = {}) {
  const urlKey = normalizeUrl(url);
  if (!urlKey) throw new Error(`unkeyable URL: ${url}`);
  const path = ledgerPath({ root, logPath, spec });
  return withPipelineLock(path, async () => {
...
export function statusFor(record, trackerRows) {
 ...
 */
export function buildReport(state, trackerRows, arms = ARMS) {
  const groups = {};
