...
export const OPS = Object.freeze({
  ...
  set_range: 'browser_click the slider, then browser_press_key Arrow* to step it to the value',
});
 ...
 */
const AUTOFILL_UPLOAD_RE = /resume|cv\b|curriculum|autofill|auto.fill|prefill|pre.fill|parse|remplir/i;
 ...
 */
/**
 * Input types whose value is typed, never chosen. `range` is absent on purpose
 * — it is a slider and has its own op — and so are `checkbox` and `radio`,
 * which are decided by role above this check.
 */
const FREE_TEXT_INPUT_TYPES = new Set([
  'email', 'tel', 'url', 'number', 'date', 'datetime-local', 'month', 'week', 'time', 'search', 'password',
]);

export function opFor(item) {
  if (item.kind === 'upload') return 'upload';
  // A radio group whose every option is unclickable, behind one visible
  // "Select an option" control, is a picklist built out of radios. Clicking an
  // option clicks something that is not rendered yet, so it has to be opened
  // first — the same treatment an ARIA combobox gets, for the same reason.
  if (item.kind === 'group' && item.collapsed) return 'expand_then_pick';
  // A range slider reports the same role as a number box (spinbutton) and
  // takes no typed value at all: a programmatic write is rejected, and the
  // only thing that moves it is a real interaction. Stepped with arrow keys
  // from its current position.
  if (item.type === 'range') return 'set_range';
  if (item.kind === 'group') return 'click';
  if (item.role === 'checkbox') return 'click';
  if (item.role === 'radio') return 'click';
  // An <input> whose HTML type declares the SHAPE of its value — email, tel,
  // url, a number, a date — is free text, whatever ARIA role the page paints
  // on it. Such a field may still offer suggestions, and a suggestion list is
  // not a closed set: a live email box carried a datalist of mail domains
  // ("aol.com", "free.fr"), was read as a picklist, and the planner refused to
  // type the candidate's own address because it was not one of the options.
  // A real picklist is never type="email".
  if (item.tag === 'input' && FREE_TEXT_INPUT_TYPES.has(item.type)) return 'fill';
...
/**
 * Does this answer mean yes?
 *
 * Used only for consent controls, where the answer is an agreement rather than
 * a value to look up in a list. Deliberately a closed list of affirmatives:
 * anything unrecognised is NOT treated as consent, because the failure
 * direction matters — a missed tick costs a retry, a wrong tick agrees to
 * something in the user's name.
 */
export function isAffirmative(value) {
  if (value === true) return true;
  const v = String(value ?? '').trim().toLowerCase();
  return ['yes', 'y', 'true', 'oui', 'ja', 'si', 'accept', 'accepted', 'agree', 'agreed',
    "j'accepte", 'jaccepte', 'i accept', 'i agree', 'on', 'checked', '1'].includes(v);
}
...
export function matchAnswer(item, answers) {
  ...
  // A TRUNCATED question still names its field.
  //
  // The orchestrator trims long labels before showing them to the model, so a
  // 300-character consent label arrives as its first 50 characters. The model
  // then answers using the text it was given, and that answer has to find its
  // way back to the full label. Word counting cannot do it — the trimmed
  // version is missing most of the words — so a clean prefix counts as a
  // match, provided it is long enough to be unambiguous.
  //
  // This is a patch over a smell: the model should be answering by index, the
  // way it already picks click targets by index, and then no text matching
  // would be needed at all.
  const MIN_PREFIX = 20;
  const prefix = list.find((a) => {
    const k = normalizeQuestion(a.question);
    if (k.length < MIN_PREFIX) return false;
    return q.startsWith(k) || k.startsWith(q);
  });
  if (prefix) return prefix;

  const contains = list.find((a) => {
    const k = normalizeQuestion(a.question);
    return k.length > 3 && labelsAgree(q, k);
  });
  return contains ?? null;
}

/**
 * How many extra words a rendered label may carry over the answer's key.
 *
 * A rendered question really can be a short sentence built around the key:
 * "How many years of experience do you have?" against a key of "years of
 * experience" adds five words and is a correct match. What it is never is a
 * paragraph — the panel that forced this rule added about thirty.
 *
 * Five, or twice the key's length if that is more, capped at ten.
 */
const extraWordsAllowed = (keyWordCount) => Math.min(Math.max(5, keyWordCount * 2), 10);

/**
 * Do these two labels name the SAME field?
 *
 * Plain containment is not enough, and a length ratio is a blunt stand-in for
 * the real question. What actually matters is whether one label is the other
 * plus a little decoration, or whether the key merely APPEARS SOMEWHERE in a
 * much larger piece of text.
 *
 * The case that forced this: a required consent tickbox whose label recited
 * its entire panel — "Créez votre compte … Métier Localité Email Type de
 * contrat CDI CDD Intérim …". The word "Email" is in there, so containment
 * matched, and the tickbox was handed the candidate's email address.
 *
 * So: every word of the shorter label must appear as a WORD in the longer one
 * (not as a fragment inside another word), and the longer one may add only a
 * couple of words. "Nom" still matches "Nom de famille"; "Email" does not
 * match a sixty-word panel.
 *
 * @param {string} a - normalized label
 * @param {string} b - normalized label
 * @returns {boolean}
 */
export function labelsAgree(a, b) {
  ...
  return longer.length - shorter.length <= extraWordsAllowed(shorter.length);
}
...
/**
 * Words that mean "the top of this scale".
 *
 * Used only for a range control, where the answer engine's label ("Fluent",
 * "C1") cannot be typed and the only expressible answer is a number. A
 * top-of-scale word maps to max; anything else is reported for judgment
 * rather than guessed, because maxing a slider that measures years or salary
 * would be a claim the plan made up.
 */
const TOP_OF_SCALE = /^(fluent|courant|native|bilingual|expert|advanced|avanc|professional|professionnel|c1|c2|yes|oui|excellent|tr.s bien)/i;

/**
 * The number to move a slider to.
 *
 * @param {object} item - the pending field, carrying min/max/step.
 * @param {{value?: string, choices?: string[]}} answer
 * @returns {{value: string|null, reason: string}}
 */
export function resolveRange(item, answer) {
  const direct = String(answer?.value ?? '').trim();
  if (/^-?\d+(\.\d+)?$/.test(direct)) return { value: direct, reason: 'answer is already a number' };

  const candidates = [direct, ...(Array.isArray(answer?.choices) ? answer.choices : [])].filter(Boolean);
  const top = candidates.some((c) => TOP_OF_SCALE.test(String(c).trim()));
  if (top && item.max !== undefined) {
    return { value: String(item.max), reason: 'a top-of-scale answer on a slider means its maximum' };
  }
  return { value: null, reason: 'a slider needs a number, and this answer is neither numeric nor top-of-scale' };
}
...
/**
 * Keep only the controls that belong to the application form.
 *
 * A careers page carries other forms: a newsletter signup in the footer, a
 * site search, a cookie panel. One live page offered a footer field labelled
 ...
 *
 * Which form is the application is decided by evidence, not by a name: the one
 * holding the most of the work (its uploads, its required fields, its groups).
 * Controls that belong to no form at all are always kept, because plenty of
 * ATS render their fields outside a <form> element entirely — dropping those
 * would empty the plan on exactly the pages that need it most.
 *
 * @param {object[]} items
 * @param {{required: object[], uploads: object[]}} work
 * @returns {object[]}
 */
function sameFormAs(items, work) {
  const weight = new Map();
  const bump = (i, by) => {
    if (typeof i !== 'number' || i < 0) return;
    weight.set(i, (weight.get(i) ?? 0) + by);
  };
  // An upload is the strongest evidence: a newsletter box never asks for a CV.
  for (const u of work.uploads ?? []) bump(u.formIndex, 3);
  for (const r of work.required ?? []) bump(r.formIndex, 2);
  for (const it of items) bump(it.formIndex, 1);
  if (weight.size <= 1) return items;

  let best = -1;
  let bestScore = -1;
  for (const [idx, score] of weight) {
    if (score > bestScore) { best = idx; bestScore = score; }
  }
  // A required CONSENT control is never dropped by form scoping, wherever it
  // lives.
  //
  // One live application put its name, email and CV in one <form> and the
  // "I accept creating an account" tickbox — required, and sitting directly
  // above the send button in the same visible card — in a sibling. Scoping by
  // form element discarded it, the page looked complete to every check, and
  // the submit would have been refused for an unticked box nobody could see
  // was missing. On that site creating the account IS applying; the page says
  // so in its own heading.
  //
  // Narrowed to CONSENT deliberately. "Any required control" is too much: the
  // mailing list this scoping exists to avoid was a required text input
  // labelled "Email address without domain", and keeping that puts the
  // candidate's address on a newsletter. A consent tickbox asks permission; a
  // data field asks for data, and a data field outside the application form is
  // not the application's.
  const kept = items.filter((it) => (it.required && phaseOf(it) === 'consent')
    || typeof it.formIndex !== 'number' || it.formIndex < 0 || it.formIndex === best);

  // An upload does not always live in the same <form> as the fields it belongs
  // to. One live posting put the name/email inputs in one form and the CV file
  // input in its sibling, so scoping to the winning form removed the upload
  // entirely and left a plan that would submit an application with no CV
  // attached — the form's own required field, silently unfilled.
  //
  // So: the scope may never be the reason NO upload survives. If the page asks
  // for a file and the filter removed every one of them, the uploads come back.
  // When an upload is already inside the winning form this changes nothing,
  // which is the ordinary case.
  const uploads = work.uploads ?? [];
  if (uploads.length && !kept.some((it) => it.kind === 'upload')) {
    const rescued = items.filter((it) => it.kind === 'upload' && !kept.includes(it));
    return [...kept, ...rescued];
  }
  return kept;
}
 ...
 */
/**
 * The pending work that belongs to the APPLICATION, with the page's other
 * forms scoped out — the same view `buildFillPlan` acts on.
 *
 * Exported because a caller that reports "what is still blocking us" from the
 * unscoped `pendingWork` tells the truth about the page and a lie about the
 * application: it lists a newsletter box and an account widget the planner
 * will never touch. A controller shown a blocker it has no power to clear
 * eventually abandons the posting, which is exactly what one live run did with
 * a fully filled, gate-passing form in front of it.
 *
 * @param {object} inventory
 * @returns {{required: object[], optional: object[], uploads: object[]}}
 */
export function applicationWork(inventory) {
  const work = pendingWork(inventory);
  const all = [...work.required, ...work.optional, ...work.uploads];
  const kept = new Set(sameFormAs(all, work));
  return {
    required: work.required.filter((i) => kept.has(i)),
    optional: work.optional.filter((i) => kept.has(i)),
    uploads: work.uploads.filter((i) => kept.has(i)),
  };
}
...
export function buildFillPlan(inventory, answers = [], options = {}) {
  ...
  const items = sameFormAs(all, work);
  ...
  for (const item of items) {
    ...
    if (op === 'click' || op === 'select_option' || op === 'expand_then_pick') {
      ...
      // A consent TICKBOX, as opposed to a consent yes/no pair.
      //
      // A single tickbox reported as a group has one meaningful choice and its
      // label is the consent sentence itself — "J'accepte de créer un compte
      // …". Two things go wrong if that is treated as a list to choose from:
      //
      //   - "yes" matches nothing, because the word "yes" does not appear in
      //     the sentence, so a genuine agreement is dropped; and worse,
      //   - "no" matches the ONLY option there is, so a REFUSAL ticks the box.
      //     That is agreeing to something in the user's name because they said
      //     they did not want to.
      //
      // So a tickbox is decided by whether the answer means yes, and by
      // nothing else. A real pair ("Yes" / "No" as separate options) is left to
      // the ordinary matching below, which can tell those two apart properly.
      // Blank option labels are ignored when deciding this.
      //
      // A page can carry a hidden twin of the same checkbox — same `name`, no
      // label — and the reader groups the two together. The group then has one
      // long consent sentence and one EMPTY option, so a test demanding that
      // every option be long said "not a tickbox", fell through to option
      // matching, and the box went untouched while the form refused to submit.
      const labelled = Array.isArray(item.options) ? item.options.map(String).filter((o) => o.trim()) : [];
      const consentTickbox = phase === 'consent'
        && (!Array.isArray(item.options) || (labelled.length > 0 && labelled.every((o) => o.length >= 40)));
      if (consentTickbox) {
        // The ORIGINAL answer, not what resolveChoice mapped it onto. Given a
        // single option, resolveChoice returns that option's text — which for
        // a tickbox is the consent sentence — so testing its output asks
        // whether the sentence means yes, which it never does.
        const said = answer.value ?? answer.choices?.[0] ?? choice.value;
        if (!isAffirmative(said)) {
          unanswered.push({
            ...item,
            phase,
            reason: `consent not given — the answer was ${JSON.stringify(said)}, and a box is never ticked on anything but a yes`,
          });
          continue;
        }
        // Prefer a choice that actually has a label: the blank one is the
        // hidden twin, and clicking that is what times out.
        const pick = Array.isArray(item.choices)
          ? (item.choices.find((c) => !c.disabled && String(c.label ?? '').trim())
            ?? item.choices.find((c) => !c.disabled))
          : null;
        actions.push({
          op: 'click',
          phase,
          target: pick?.clickSelector || pick?.selector || item.clickSelector || item.selector,
          value: 'yes',
          question: item.question,
          note: 'consent tickbox: the label IS the option, so this is decided by the answer meaning yes.',
          verify: 'after this, the partner control must read disabled:true, required:false',
        });
        continue;
      }
      ...
      if (item.kind === 'group' && item.collapsed) {
        actions.push({
          op, phase, target: item.expandSelector, value: choice.value, ranked: choice.ranked,
          question: item.question,
          ...(item.menuSelector ? { menu: item.menuSelector } : {}),
          alreadyExpanded: item.expandedNow === true,
          note: item.expandedNow === true
            ? 'ALREADY OPEN — do not click the opener, it is a toggle and a second click closes it. Read the options and click the one matching value.'
            : 'click the opener ONCE (it is a toggle), then read the options — they may live in the aria-controls target rather than the group, and may be buttons rather than labels — then click the one matching value',
        });
        continue;
      }

      // A hidden control needs its visible label clicked instead; the reader
      // reports one only when the control itself cannot be clicked.
      let target = item.clickSelector || item.selector;
      if (item.kind === 'group' && Array.isArray(item.choices)) {
        ...
        target = pick.clickSelector || pick.selector;
      }
      actions.push({
    ...
    if (op === 'set_range') {
      const ranged = resolveRange(item, answer);
      if (!ranged.value) { unanswered.push({ ...item, phase, reason: ranged.reason }); continue; }
      actions.push({
        op, phase, target: item.clickSelector || item.selector, value: ranged.value,
        question: item.question,
        ...(item.min !== undefined ? { min: item.min } : {}),
        ...(item.max !== undefined ? { max: item.max } : {}),
        ...(item.value !== undefined ? { from: item.value } : {}),
        note: `${ranged.reason}; click the slider, then step it with ArrowRight/ArrowLeft — a programmatic write is refused`,
      });
      continue;
    }
