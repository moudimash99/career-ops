...
/**
 * Is there anything to calibrate the user's actual voice against?
 *
 * This check is the honest limit of the whole file, and it is worth stating at
 * the point of use rather than in a doc nobody opens. Everything else here
 * removes the tells of machine writing. None of it makes prose sound like a
 * PARTICULAR person, and it cannot: that needs a sample of how they write.
 *
 * `modes/_writing.md` defines the calibration path — read `writing-samples/`,
 * extract tone and structure, cache it in `modes/_profile.md` under
 * `## Writing Style`. When neither exists, generated prose lands on
 * `voice-dna.md`'s generic register, which is a good default and still not the
 * candidate's own. Passing this checker is therefore necessary and not
 * sufficient, and a caller that mistakes "clean" for "sounds like me" has been
 * misled by a green result.
 *
 * @param {string} [root]
 * @returns {{calibrated: boolean, samples: number, cached: boolean}}
 */
export function styleCalibration(root = getCareerOpsRoot()) {
  let samples = 0;
  try {
    // README.md in that directory is the shipped explainer, not a sample —
    // modes/_writing.md excludes it by name and so must this count, or the
    // directory looks calibrated when it is empty.
    samples = readdirSync(join(root, 'writing-samples'))
      .filter((f) => f.toLowerCase() !== 'readme.md').length;
  } catch { samples = 0; }

  let cached = false;
  try {
    cached = /^##\s+Writing Style/m.test(readFileSync(join(root, 'modes/_profile.md'), 'utf-8'));
  } catch { cached = false; }

  return { calibrated: samples > 0 || cached, samples, cached };
}
...
function main(argv) {
  ...
  if (hasFlag(argv, '--json')) {
    console.log(JSON.stringify({ ...result, voiceDna: rules.found ? rules.path : null, calibration }, null, 2));
  ...
  // Said on every human-readable run, clean or not: "passes the checker" and
  // "sounds like the candidate" are different claims, and only the first is
  // being made here. NOT in --json mode, where stdout has to stay parseable —
  // the same field is on the JSON object as `calibration`.
  if (!calibration.calibrated && !hasFlag(argv, '--json')) {
    console.log('\nnote: no writing sample to calibrate against (writing-samples/ is empty and');
    console.log('      modes/_profile.md has no "## Writing Style"). This text is de-slopped');
    console.log('      against voice-dna.md, NOT matched to how you actually write. Drop one');
    console.log('      past cover letter or LinkedIn About into writing-samples/ to fix that.');
  }
