...
/** Blacklist Map → (capKey → entry), built once per loaded blacklist. */
const blacklistByCapKey = new WeakMap();

/**
 * Find the do-not-apply entry a posting's company falls under.
 *
 * data/blacklist.md names employers ("Accenture"), while postings arrive under
 * whatever the feed calls them ("Accenture France", "NTT DATA", "Thales Alenia
 * Space"). An exact normalized key missed all of those: a scan on 2026-09-11
 * put three "Accenture France" postings into the pipeline the day after the
 * user blacklisted Accenture, and the applier's own gate used the same lookup.
 *
 * Exact normalized name first, so every row that matched before still matches.
 * Then the same employer resolution the cap uses: corporate suffixes stripped,
 * and the blacklisted name accepted as a leading whole-token prefix. Anchored
 * at the start, so a posting that merely mentions a blacklisted company later
 * in its name is not caught.
 *
 * @param {Map<string, {company: string}>} blacklist - From scan.mjs loadBlacklist/parseBlacklist.
 * @param {string} company - Company name as the posting gives it.
 * @returns {object|null} The blacklist entry, or null when the company is not listed.
 */
export function matchBlacklist(blacklist, company) {
  if (!blacklist || blacklist.size === 0) return null;
  const exact = blacklist.get(normalizeCompany(String(company ?? '')));
  if (exact) return exact;

  const key = capKey(company);
  if (!key) return null;
  let byKey = blacklistByCapKey.get(blacklist);
  if (!byKey) {
    byKey = new Map();
    for (const entry of blacklist.values()) {
      const entryKey = capKey(entry.company);
      if (entryKey && !byKey.has(entryKey)) byKey.set(entryKey, entry);
    }
    blacklistByCapKey.set(blacklist, byKey);
  }
  const match = resolveCountedKey(key, byKey);
  return match ? byKey.get(match.key) : null;
}
